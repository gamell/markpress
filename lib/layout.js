...
function random(params) {
  ...
  for (let i = 0; i < numOfParams; i++) {
    ...
    const param = params.splice(util.getRandomInt(0, params.length - 1), 1)[0];
...
const random7d = () =>
  random(["x", "y", "z", "scale", "rotate-x", "rotate-y", "rotate-z"]);
...
function generateNewCoord(lastCoord) {
  ...
  direction =
    direction % 2 === lastCoord.direction % 2 ? (direction + 1) % 2 : direction;
