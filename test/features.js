...
const inputEmbeddedOptions = path.resolve(
  __dirname,
...
const generateHtml = (options, done) => runMarkpress(input, options, done);
...
describe("markpress feature test", function test() {
  ...
  describe("Sanitize: off", () => {
    ...
    it("should not remove HTML", () => {
      assert.include(
        ...
        "HTML contains correct native HTML tags"
      );
    });
  ...
  describe("Image embed: On", () => {
    ...
    it("should not contain the remote image url", () => {
      assert.notInclude(
        html,
        "http://www.canbike.org/public/images/030114/Bitcoin_Logo.png"
      );
    });
    ...
    it("should contain local embedded image", () => {
      assert.include(
        html,
        "/4CkYFhJeWFjarSFgTxPVsG80LCCKbhyzjxyUkIhOSEpKODQAA"
      );
    });
  });
  ...
  describe("Feature Support", () => {
    ...
    it("should support Tables", () => {
      assert.include(html, "<table>", "HTML contains table tag");
    });
    it("should contain Print CSS", () => {
      assert.include(
        html,
        '<style media="print"',
        "HTML contains Print CSS tag"
      );
      assert.include(
        html,
