...
const baseArgv = ["/path/to/bin/node", "/path/to/bin/markpress.js", "input.md"];
...
function getArgs(additionalArgs) {
  const args = baseArgv.slice(0); // cloning the base array https://davidwalsh.name/javascript-clone-array
  return args.concat(additionalArgs);
}

const assertOptsForArgs = (args, expectedOpts, done) =>
  markpressExecutor(getArgs(args))
    .then((exit) => {
...
describe("MarkpressExecutor (CLI)", function test() {
  beforeEach(() => {
    ...
    markpressStub = sandbox
      .stub()
      .returns(Promise.resolve({ html: "nothing" }));
    const freshProgram = rewire("commander");
    markpressExecutor.__set__({
      markpress: markpressStub,
  ...
  describe("Layout Option Parsing", () => {
    it("it should correctly parse option shorthand", (done) => {
      assertOptsForArgs(
        ["-l", "grid"],
        { autoSplit: undefined, layout: "grid", verbose: true },
        done
      );
    });
    it("it should correctly parse option longhand", (done) => {
      assertOptsForArgs(
        ["--layout", "3d-push"],
        { autoSplit: undefined, layout: "3d-push", verbose: true },
        done
      );
    });
  });
  ...
  describe("Theme Option Parsing", () => {
    it("it should correctly parse option shorthand", (done) => {
      assertOptsForArgs(
        ["-t", "dark"],
        { sanitize: undefined, layout: undefined, theme: "dark" },
        done
      );
    });
    it("it should correctly parse option longhand", (done) => {
      assertOptsForArgs(
        ["--theme", "light"],
        { sanitize: undefined, layout: undefined, theme: "light" },
        done
      );
    });
  });
  ...
  describe("Sanitize Option Parsing", () => {
    it("it should correctly parse option shorthand", (done) => {
      assertOptsForArgs(
        ["-z"],
        { theme: undefined, autoSplit: undefined, sanitize: true },
        done
      );
    });
    it("it should correctly parse option longhand", (done) => {
